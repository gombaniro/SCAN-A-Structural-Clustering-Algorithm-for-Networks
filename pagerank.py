"""pagerank.py illustraes how to use the pregel.py library, and tests
that the library works.

It illustrates pregel.py by computing the PageRank for a randomly
chosen 10-vertex web graph.

It tests pregel.py computing the PageRank for the same graph in a different
,more conventional way, and showing that the two outputs are
near-identical."""
...
def main():
    ...
    print("The norm of the difference is :",linalg.norm(diff))

def create_edges(vertices):
          """ Generates 4 randomly chosen outgoing edges from each vertex in vertices."""
          for vertex in vertices:
               vertex.neighbors = random.sample(vertices,4)
...
def pagerank_test(vertices):
          ...
          for vertex in vertices:
              num_neighbors = len(vertex.neighbors)
              for out_vertex in vertex.neighbors:
                  G[out_vertex.id,vertex.id] = 1.0/num_neighbors 
...
class PageRankVertex(Vertex):
    ...
    def update(self):
          # This routing has a bug when there pages with no outgoing
          #links (never the case for our tests). This problem can be
          #solved by indroducing Aggregators into the Pregel framework,
